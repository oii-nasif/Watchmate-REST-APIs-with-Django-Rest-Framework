...
class StreamPlatformTestCase(APITestCase):
    ...
    def test_streamplatform_update(self):
        ...
        response = self.client.put(reverse('stream-list'), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_streamplatform_delete(self):
        ...
        response = self.client.delete(reverse('stream-list'), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class ReviewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.stream = models.StreamPlatform.objects.create(name="Netflix", about="#1 Streaming Platform", website="https://www.netflix.com")
        self.watchlist = models.WatchList.objects.create(platform=self.stream, title="Example Movie", storyline="Example Story", active=True)
        self.review = models.Review.objects.create(review_user=self.user, rating=5, description="Good Movie", watchlist=self.watchlist, active=True)
